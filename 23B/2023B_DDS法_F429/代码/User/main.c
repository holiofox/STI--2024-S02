 ...
 */
 ...
 SemaphoreHandle_t BinarySem_Handle_Tim =NULL;
...
int findMinIndex(uint32_t arr[], int start, int end);
...
uint16_t L_tag=0;
uint32_t AMP[2000];
uint32_t AMP_NEXT[4000];
uint32_t MAX_FREQ,MIN_FREQ,index1,index2,index3;
extern uint8_t abc;
...
static void AppTaskCreate(void)
{
  ...
  if(NULL != BinarySem_Handle_TR)
	...
	BinarySem_Handle_Tim = xSemaphoreCreateBinary();	
	  if(NULL != BinarySem_Handle_Tim)
    printf("BinarySem_Handle_Tim二值信号量创建成功!\r\n");
...
static void L_Task(void* parameter)
{
  ...
  while (1)
  {
    ...
    TEXT_SetText(hItem, "\xE6\xAD\xA3\xE5\x9C\xA8\xE6\xA3\x80\xE6\xB5\x8B");
		...
		memset(AMP_NEXT,0,sizeof(AMP_NEXT));			
		memset(buffer_L,0,sizeof(buffer_L));
		l = getL();
		if(l<10)
		l = 0.99707*l-0.021046;
		l *=100;
		switchX('N');
